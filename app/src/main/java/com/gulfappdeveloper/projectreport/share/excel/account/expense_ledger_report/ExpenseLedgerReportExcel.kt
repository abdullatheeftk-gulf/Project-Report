package com.gulfappdeveloper.projectreport.share.excel.account.expense_ledger_report
...
object ExpenseLedgerReportExcel {
    suspend fun writeExcelSheet(
        ...
        try {
            ...
            // Heading creation
            sheet.createHeading(wb = wb, headingTitle = "Expense Ledger Report", noOfColumns = 6)
            ...
            list.forEachIndexed { index, reArrangedExpenseLedgerDetails ->
                createItemRow(
                    reArrangedExpenseLedgerDetails = reArrangedExpenseLedgerDetails,
                    index = index,
            ...
            val file =
                File(context.getExternalFilesDir(null), "ExpenseLedgerReport_${Date().time}.xlsx")
    ...
    private fun createItemRow(
        reArrangedExpenseLedgerDetails: ReArrangedExpenseLedgerDetail,
        index: Int,
        isItLastRow: Boolean,
        sheet: Sheet,
        wb: XSSFWorkbook
        ...
        try {
            ...
            // Si
            headerRow.createCell(0).apply {
                setCellValue(reArrangedExpenseLedgerDetails.si.toString())
                cellStyle = wb.createCellStyle().apply {
                    borderRight = BorderStyle.THIN
                    borderBottom = if (isItLastRow) BorderStyle.MEDIUM else BorderStyle.THIN
                    borderLeft = BorderStyle.MEDIUM
            ...
            // Date
            headerRow.createCell(1).apply {
                setCellValue(reArrangedExpenseLedgerDetails.voucherDate.stringToDateStringConverter())
                cellStyle = generalHeaderCellStyle
            }
            // invoice No
            headerRow.createCell(2).apply {
                setCellValue(reArrangedExpenseLedgerDetails.voucherNo.toString())
                cellStyle = generalHeaderCellStyle
            }
            // Party
            headerRow.createCell(3).apply {
                setCellValue(reArrangedExpenseLedgerDetails.particulars ?: "--")
                cellStyle = generalHeaderCellStyle
            }
            // Debit
            headerRow.createCell(4).apply {
                setCellValue(reArrangedExpenseLedgerDetails.debit.toDouble())
            ...
            // Total
            headerRow.createCell(5).apply {
                setCellValue(reArrangedExpenseLedgerDetails.credit.toDouble())
                cellStyle = wb.createCellStyle().apply {
                    borderRight = BorderStyle.MEDIUM
                    borderBottom = if (isItLastRow) BorderStyle.MEDIUM else BorderStyle.THIN
                    alignment = HorizontalAlignment.CENTER
