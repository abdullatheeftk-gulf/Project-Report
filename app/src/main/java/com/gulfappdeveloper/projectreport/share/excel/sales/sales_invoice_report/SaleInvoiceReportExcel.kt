package com.gulfappdeveloper.projectreport.share.excel.sales.sales_invoice_report
...
object SaleInvoiceReportExcel {
    suspend fun writeExcelSheet(
        ...
        try {
            val wb = XSSFWorkbook()
            val currentDateAndTime = SimpleDateFormat("dd-MM-yyyy hh;mm;ss a", Locale.getDefault()).format(
                Date()
            )
            val sheet = wb.createSheet(currentDateAndTime)

            // Heading creation
            sheet.createHeading(wb = wb, headingTitle = "Sales Invoice Report", noOfColumns = 9)
            // Period
            ...
            val file =
                File(context.getExternalFilesDir(null), "SalesInvoiceReport_${Date().time}.xlsx")
    ...
    private fun createPeriodTitleAndCompanyName(
        ...
        try {
            ...
            val richTextString = XSSFRichTextString("Period : $fromDate, $fromTime to $toDate, $fromTime").apply {
                val lengthOfFromDate = "$fromDate, $fromTime".length
                val totalLength = "Period : $fromDate, $fromTime to $toDate, $fromTime".length - 1
    ...
    private fun createItemRow(
        ...
        // invoice No
        headerRow.createCell(2).apply {
            setCellValue(salesInvoiceResponse.invoiceNo.toString())
            cellStyle = generalHeaderCellStyle
        }
        // Account
        headerRow.createCell(3).apply {
            setCellValue(salesInvoiceResponse.party ?: "--")
            cellStyle = generalHeaderCellStyle
        }
        // Taxable
        headerRow.createCell(4).apply {
            setCellValue(salesInvoiceResponse.taxable.toDouble())
        ...
        // Tax
        headerRow.createCell(5).apply {
            setCellValue(salesInvoiceResponse.tax.toDouble())
            cellStyle = generalHeaderCellStyle.apply {
                dataFormat = wb.createDataFormat().getFormat("0.00")
            }
        }
        // Return
        headerRow.createCell(6).apply {
            setCellValue(salesInvoiceResponse.returnTaxable.toDouble())
            cellStyle = generalHeaderCellStyle.apply {
                dataFormat = wb.createDataFormat().getFormat("0.00")
            }
        }
        // Tax on Return
        headerRow.createCell(7).apply {
            setCellValue(salesInvoiceResponse.taxOnReturn.toDouble())
            cellStyle = generalHeaderCellStyle
        }

        // Total
        headerRow.createCell(8).apply {
            setCellValue(salesInvoiceResponse.net.toDouble())
            cellStyle = wb.createCellStyle().apply {
                borderRight = BorderStyle.MEDIUM
                borderBottom = if (isItLastRow) BorderStyle.MEDIUM else BorderStyle.THIN
                alignment = HorizontalAlignment.CENTER
