package com.gulfappdeveloper.projectreport.share.excel.account.payements_report
...
object PaymentsReportExcel {
    suspend fun writeExcelSheet(
        ...
        try {
            ...
            // Heading creation
            sheet.createHeading(wb = wb, headingTitle = "Payments Report", noOfColumns = 5)
            // Period
            ...
            list.forEachIndexed { index, paymentsResponse ->
                createItemRow(
            ...
            val file =
                File(context.getExternalFilesDir(null), "PaymentsReport_${Date().time}.xlsx")
    ...
    private fun createItemRow(
        ...
        // voucher no
        headerRow.createCell(2).apply {
            setCellValue(paymentResponse.vchrNo.toString())
            cellStyle = generalHeaderCellStyle
        }
        // Particulars
        headerRow.createCell(3).apply {
            setCellValue(paymentResponse.particulars)
            cellStyle = generalHeaderCellStyle
        }

        // Amount
        headerRow.createCell(4).apply {
            setCellValue(paymentResponse.amount.toDouble())
            cellStyle = wb.createCellStyle().apply {
                borderRight = BorderStyle.MEDIUM
                borderBottom = if (isItLastRow) BorderStyle.MEDIUM else BorderStyle.THIN
                alignment = HorizontalAlignment.CENTER
