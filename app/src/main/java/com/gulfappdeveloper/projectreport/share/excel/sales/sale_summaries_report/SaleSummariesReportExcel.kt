package com.gulfappdeveloper.projectreport.share.excel.sales.sale_summaries_report
...
object SaleSummariesReportExcel {
    suspend fun writeExcelSheet(
        ...
        try {
            ...
            // Heading creation
            sheet.createHeading(wb = wb, headingTitle = "Sales Summaries Report", noOfColumns = 7)
            // Period
            ...
            list.forEachIndexed { index, saleSummariesResponse ->
                createItemRow(
                    saleSummariesResponse = saleSummariesResponse,
                    index = index,
    ...
    private fun createItemRow(
        ...
        // Date
        headerRow.createCell(1).apply {
            setCellValue(saleSummariesResponse.date.stringToDateStringConverter())
            cellStyle = generalHeaderCellStyle
        }

        // Taxable
        headerRow.createCell(2).apply {
            setCellValue(saleSummariesResponse.taxable.toDouble())
        ...
        // Tax
        headerRow.createCell(3).apply {
            setCellValue(saleSummariesResponse.tax.toDouble())
            cellStyle = generalHeaderCellStyle.apply {
                dataFormat = wb.createDataFormat().getFormat("0.00")
            }
        }
        // Return
        headerRow.createCell(4).apply {
            setCellValue(saleSummariesResponse.returnTaxable.toDouble())
            cellStyle = generalHeaderCellStyle.apply {
                dataFormat = wb.createDataFormat().getFormat("0.00")
            }
        }
        // Tax on Return
        headerRow.createCell(5).apply {
            setCellValue(saleSummariesResponse.returnTax.toDouble())
            cellStyle = generalHeaderCellStyle
        }

        // Total
        headerRow.createCell(6).apply {
            setCellValue(saleSummariesResponse.net.toDouble())
            cellStyle = wb.createCellStyle().apply {
                borderRight = BorderStyle.MEDIUM
                borderBottom = if (isItLastRow) BorderStyle.MEDIUM else BorderStyle.THIN
                alignment = HorizontalAlignment.CENTER
