package com.gulfappdeveloper.projectreport.share.excel.sales.customer_ledger_report
...
object CustomerLedgerReportExcel {
    suspend fun writeExcelSheet(
        ...
        try {
            ...
            // Heading creation
            sheet.createHeading(wb = wb, headingTitle = "Customer Ledger Report", noOfColumns = 6)
            ...
            val file =
                File(context.getExternalFilesDir(null), "CustomerLedgerReport_${Date().time}.xlsx")
    ...
    private fun createItemRow(
        ...
        try {
            ...
            // Si
            headerRow.createCell(0).apply {
                setCellValue(reArrangedCustomerLedgerDetails.si.toString())
                cellStyle = wb.createCellStyle().apply {
                    /*fillForegroundColor = IndexedColors.GOLD.index
                fillPattern = FillPatternType.SOLID_FOREGROUND*/
                    borderRight = BorderStyle.THIN
            ...
            // invoice No
            headerRow.createCell(2).apply {
                setCellValue(reArrangedCustomerLedgerDetails.voucherNo.toString())
                cellStyle = generalHeaderCellStyle
            }
            // Party
            headerRow.createCell(3).apply {
                setCellValue(reArrangedCustomerLedgerDetails.particulars ?: "--")
                cellStyle = generalHeaderCellStyle
            }
            // Debit
            headerRow.createCell(4).apply {
                setCellValue(reArrangedCustomerLedgerDetails.debit.toDouble())
            ...
            // Total
            headerRow.createCell(5).apply {
                setCellValue(reArrangedCustomerLedgerDetails.credit.toDouble())
                cellStyle = wb.createCellStyle().apply {
                    borderRight = BorderStyle.MEDIUM
                    borderBottom = if (isItLastRow) BorderStyle.MEDIUM else BorderStyle.THIN
                    alignment = HorizontalAlignment.CENTER
